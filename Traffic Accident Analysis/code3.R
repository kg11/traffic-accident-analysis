...
ind<-sample.split(Y=data$Vehicle_Type,SplitRatio = 0.8)
traindf<-data[ind,]
testdf<-data[!ind,]

pct<-ctree(Vehicle_Type ~ .,data=traindf,controls=ctree_control(mincriterion=0.99,minsplit=200000))
plot(pct)

pwc<-predict(pct,testdf)
pwc
t<-table(predictions=pwc,actual=testdf$Vehicle_Type)
t
sum(diag(t))/sum(t)

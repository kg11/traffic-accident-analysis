...
ind<-sample.split(Y=data$Light_Conditions,SplitRatio = 0.8)
traindf<-data[ind,]
testdf<-data[!ind,]

pct<-ctree(Light_Conditions ~ .,data=traindf,controls=ctree_control(mincriterion=0.99,minsplit=200000))
plot(pct)

pwc<-predict(pct,testdf)
pwc
t<-table(predictions=pwc,actual=testdf$Light_Conditions)
t
sum(diag(t))/sum(t)

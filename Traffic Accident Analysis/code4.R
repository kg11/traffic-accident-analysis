...
ind<-sample.split(Y=data$Urban_or_Rural_Area,SplitRatio = 0.8)
traindf<-data[ind,]
testdf<-data[!ind,]

pct<-ctree(Urban_or_Rural_Area ~ .,data=traindf,controls=ctree_control(mincriterion=0.99,minsplit=200000))
plot(pct)

pwc<-predict(pct,testdf)
pwc
t<-table(predictions=pwc,actual=testdf$Urban_or_Rural_Area)
t
sum(diag(t))/sum(t)
